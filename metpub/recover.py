...
def main(args=None) -> None:
    ...
    # Step 3: Load and validate API configurations
    try:
    ...
    # Step 6: Perform recovery and stream file download
    req = urllib.request.Request(recover_url)
    req.add_header("Authorization", f"Bearer {api_key}")

    try:
        with urllib.request.urlopen(req) as response:
            # Note: urlopen raises HTTPError for non-2xx/3xx response codes
